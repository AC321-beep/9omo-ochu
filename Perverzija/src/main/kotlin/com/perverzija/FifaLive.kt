...
class FifaLive : MainAPI() {
    ...
    // Keywords to filter strictly for FIFA content
    private val fifaKeywords = listOf("fifa", "world cup", "football")

    private fun isFifaRelated(text: String?): Boolean {
        if (text.isNullOrBlank()) return false
        return fifaKeywords.any { text.contains(it, ignoreCase = true) }
    }

    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        val req = app.get("$mainUrl/app.php", headers = apiHeaders)
        val res = req.parsedSafe<CloudPlayResponse>()
            ?: throw Error("Failed to fetch server config. Text: ${req.text}")

        val decryptedJson = decryptPayload(res.payload, res.iv)
        val streams = parseJson<CloudPlayStreams>(decryptedJson).streams

        // Filter and map ONLY FIFA streams/channels
        val homePageLists = streams.mapNotNull { stream ->
            val allShows = fetchChannels(stream.url, stream.logo)
            
            // Keep a show if its name is FIFA related, or if the parent stream category is FIFA related
            val fifaShows = allShows.filter { show ->
                isFifaRelated(show.name) || isFifaRelated(stream.name)
            }

            if (fifaShows.isNotEmpty()) {
                HomePageList(stream.name ?: "FIFA Events", fifaShows, isHorizontalImages = true)
            } else {
                null
            }
        }
    ...
    private fun parseM3u(m3uText: String): List<CloudPlayChannel> {
        ...
        for (line in lines) {
            ...
            if (l.startsWith("#EXTINF:")) {
                ...
                if (opt.startsWith("http-user-agent=")) currentUserAgent = opt.substringAfter("=")
                else if (opt.startsWith("http-referrer=")) currentReferer = opt.substringAfter("=")
                ...
                channels.add(CloudPlayChannel(
                    type = type, id = null, name = currentName, group = currentGroup,
                    logo = currentLogo, user_agent = urlUserAgent,
                    m3u8_url = if (type == "hls") rawUrl else null,
                    mpd_url = if (type == "dash") rawUrl else null,
                    license_url = licenseUrl, headers = headersMap
                ))

                currentName = ""; currentLogo = ""; currentGroup = ""
                currentKey = ""; currentKeyId = ""; currentUserAgent = ""; currentReferer = ""
            }
        }
        return channels
    }
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        val res = app.get("$mainUrl/app.php", headers = apiHeaders).parsedSafe<CloudPlayResponse>() ?: return emptyList()
        val decryptedJson = decryptPayload(res.payload, res.iv)
        val streams = parseJson<CloudPlayStreams>(decryptedJson).streams
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        return newLiveStreamLoadResponse(data.name ?: "FIFA Stream", url, url) {
    ...
    private fun String.hexToBase64Url(): String {
        ...
        if (normalizedHex.isEmpty() || normalizedHex.length % 2 != 0 || !normalizedHex.matches(Regex("^[0-9a-fA-F]+$"))) return this
        return try {
    ...
    private suspend fun getDRMKeysFromLicenseServer(url: String, kid: String): String {
        ...
        val responseString = app.post(
            url,
            headers = mapOf("User-Agent" to userAgent, "Content-Type" to "application/json;charset=UTF-8"),
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        ...
        if (channel.mpd_url != null) {
            ...
            if (licenseUrl.contains("keyid=") && licenseUrl.contains("key=")) {
                ...
                val drmKid = Regex("""cenc:default_KID=["']([0-9a-fA-F\-]{36})["']""").find(mpdStr)?.groupValues?.get(1) ?: UUID.randomUUID().toString()
    ...
    data class CloudPlayResponse(val payload: String, val iv: String, val expires: Long?)
    data class CloudPlayStreams(val streams: List<CloudPlayStream>)
    data class CloudPlayStream(val name: String?, val url: String, val logo: String?)
    data class CloudPlayChannel(
        val type: String?, val id: String?, val name: String?, val group: String?,
        val logo: String?, val user_agent: String?, val m3u8_url: String?,
        val mpd_url: String?, val license_url: String?, val headers: Map<String, String>?
    )
}
